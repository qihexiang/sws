...
lazy_static! {
    ...
    pub static ref RING_BOND_RE: Regex = Regex::new(r"^((?P<bond_type>\-|=|#|$|:|\|//)?((?P<ring_id>[1-9])|(%(<?P<ring_id>[1-9][0-9]+))))$").unwrap();
    ...
    pub static ref STANDARD_NODE_RE: Regex = Regex::new(r"^(\[(?P<isotope>[1-9][0-9]*)?(?P<element>(br?|cl?|n|o|p|s|f|i)|([A-Z][a-z]?))(?P<chirality>@{0,2})(H(?P<explicit_hydrogen>[1-9][0-9]*)?)?(?P<charge>((?P<charge_num>(\+|\-)([1-9][0-9]*))|(\+*)|(\-*)))(:(?P<react_id>[0-9]+))?(?P<selector>\{.+?\})?\])$").unwrap();
