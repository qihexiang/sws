...
impl Molecules {
    ...
    /// 根据给出的两个节点（from和to）找到对应的键，使之反向
    /// 当前还不能很好的适应手性，但是可以支持顺反异构的反向
    pub fn reverse_bond(&mut self, from: NodeIndex, to: NodeIndex) -> Option<bool> {
    ...
    /// 从给定的节点出发，将沿途所有的进入方向的键反向
    /// 该函数是递归的，必须传递上一个节点序号以防止在两个节点间反复
    pub fn reverse_recursive(&mut self, current: NodeIndex, last: Option<NodeIndex>) -> Option<()> {
